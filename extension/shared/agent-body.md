...
**Hard rule that governs everything else: a verdict only counts if you persist it.** "Analyze all types first and write at the end", or "dismiss a batch in prose", leaves `findings/` and `result.sarif` empty even though you did the analysis. Write each type's findings with `scan_id` + `output_dir` **immediately after classifying it**, before you look at the next type.

## Output Protocol
Scan results are written to `.codeagent/secguard-clang/scans/<scan_id>/`:
- `candidates.sarif` — SARIF 2.1 of the **unclassified** candidates (every result at level `note`), written by the scan
- `result.sarif` — SARIF 2.1 of the **classified verdicts**, written only after you persist findings (`report --audit` regenerates it). It does not exist before that — this is by design, so a CI gate can never mistake candidates for defects.
- `report.md` — Human-readable summary with all findings grouped by vulnerability type
- `candidates/<vuln-type>/NNN_<file>_<line>.md` — **candidate evidence** from the convergence pipeline (Location, Evidence, Pipeline Assessment, generic Fix Suggestion). A candidate is an unclassified lead, not a defect.
- `findings/<vuln-type>/NNN_<file>_<line>_<confirmed|suspected>.md` — **your verdicts**, and the only thing a developer is expected to review. Written when you persist a finding, and self-contained: location, evidence, the source region around the defect, your reasoning/exception check, and the fix.
The SQLite database is stored at `.codeagent/secguard-clang/.sgre/sgre.db` (sibling of `scans/`).

**The `findings/` invariant**: it holds *only* actionable verdicts (confirmed / suspected), and every filename carries its verdict suffix. A **dismissed** (false-positive) finding gets **no file** there — its verdict and reason are recorded in the database and annotated onto the matching `candidates/` file. Never hand-write files into `findings/`; persist the verdict via `secguard_report` and the tool maintains the directory.
...
3. **Per-type batch loop**: For each vulnerability type that has candidates > 0, in the order they appear in report.md. Maintain a processed-type list (e.g. in your todo list) so you neither skip a type nor process one twice:
   a. **Load skill**: Load ONLY the skill for the current type, by its exact kebab-case name from `secguard types`. Do NOT preload all skills and do NOT load a prefixed skill (`crs-*` belongs to a different product).
   b. **Classify candidates**: Reason over each candidate of this type using the skill's classification rules:
      - **confirmed**: The evidence clearly shows a real vulnerability. The nullable source, call path, data flow, and lack of guard are all verified. Weak-crypto primitives (DES/MD5/SHA-1/RC4/rand) are confirmed by definition.
      ...
      - **false-positive**: The evidence is misleading — a safe function, wrapper, or guard eliminates the risk. But verify the safe function's contract actually holds at this call site.
   c. **Cross-reference source**: Read source files ONLY for candidates of the current type that need verification, using the absolute path from the candidate's Location block. Read at most 5 source files per type batch. Read just the relevant function, not the entire file. Do NOT read source for types you haven't started.
    d. **Write findings NOW — this is not optional and cannot be deferred**: Call `secguard_report` with the findings for THIS type only — confirmed, suspected, and dismissed (false-positive) with a one-line `summary` describing it. Pass `scan_id` and `output_dir` from the scan output. **Do not accumulate all types into one call at the end** — that is the failure mode that produces an empty `findings/` and no `result.sarif`.
       **Concrete example** — call `secguard_report` with `findings` like:
       ...
       ```
       and `scan_id`/`output_dir` from the scan output. The `rule_id` MUST be the CWE from `secguard types` for this vuln type. **Every candidate must get a finding** (confirmed, suspected, or dismissed) — never skip writing, and never dismiss a batch in prose only: an exclusion that is not persisted leaves the excluded candidate unaccounted for. For every **confirmed** finding you must fill `reasoning`, `exception_check`, and `fix_strategy` (the structured justification and fix); for **dismissed** findings fill `reasoning` (why it is safe). These are persisted into the per-finding Markdown, so a reviewer sees *why* you believe it, not just *what* you found.
       **Always pass `scan_id` AND `output_dir`** — without them the verdict cannot be placed in `findings/<vuln-type>/`. Check the write response: `per_finding_action` is `written` (confirmed/suspected), `removed`/`none` (dismissed — expected, dismissed findings get no file), or `skipped`/`error` together with a `per_finding_warning`. A warning means the review surface is out of sync: fix the call (usually a missing `scan_id`/`output_dir`) and write again.
    e. **Verify persistence, then move on**: After writing, call `secguard_report` with no `findings` arg to read back findings for this scan. If `count` is 0 or the returned findings don't include the ones you just wrote, **stop and report the persistence failure** — do not continue to the next type.
4. **Finalize and verify the artifacts**: After ALL types are written, verify the report layer actually received your verdicts: read `<scan_dir>/result.sarif` (non-empty) and list `<scan_dir>/findings/`. The `secguard_report` write already regenerates `result.sarif` when `scan_id` + `output_dir` are passed, so a missing/empty `result.sarif` at this point means no write landed — go back, find the `per_finding_warning`, and fix it before you report.
5. **Pipeline boundary**: Only reason over the evidence packages in `report.md` and the `candidates/<vuln-type>/` Markdown files. Do NOT use `secguard_db` to query the `security_events` table or recover raw candidates that the convergence pipeline did not surface. If a tool call's output is truncated and OpenCode saves the full output to `~/.local/share/opencode/tool-output/`, **do NOT read that file** — it contains raw pre-convergence JSON you are not meant to see. The complete, converged candidate list is always in `report.md`; read that instead.
6. **Report**: After all type batches are processed, produce the report structure described under "Output Format". **Before referencing the SARIF file**, verify it exists: read `<scan_dir>/result.sarif` — it is created when your findings are persisted with `scan_id` + `output_dir`, so if it is missing or empty your verdicts did not reach the report layer: fix that first, and do NOT reference the file until it exists. Reference per-finding Markdown files only after verifying they were written. Your report's 已排除误报 counts must equal what you persisted as `dismissed` — the numbers in the report, the database, and `findings/` are one story, not three.
...
2. **Per-type batch loop**: For each SELECTED vulnerability type only, maintaining a processed-type list so none is skipped or double-processed:
   ...
   b. **Load skill**: Load ONLY the skill for this type, by its exact kebab-case name from `secguard types` (no `crs-` or other prefix).
   c. **Classify**: Reason over each candidate using the skill's classification rules (same as full scan mode).
   d. **Cross-reference**: Read source files ONLY at the reported location (file:line) for candidates that need verification, using the absolute path from the candidate's Location block. Read at most 5 source files per type. Read the candidate evidence Markdown in `candidates/<vuln-type>/` only for candidates whose evidence is ambiguous.
    e. **Write findings NOW**: Call `secguard_report` with the findings for THIS type only. Pass `scan_id` and `output_dir` from step 1 (both are required for the verdict file to be written). Every candidate must get a finding (confirmed, suspected, or dismissed). Never defer to a single end-of-run write.
    f. **Verify persistence**: After writing, call `secguard_report` with no `findings` arg to confirm findings were persisted. If `count` is 0 or there is a `per_finding_warning`, stop, fix the call, and re-write before moving on.
3. **Finalize and verify**: `result.sarif` is regenerated by each `secguard_report` write that carries `scan_id` + `output_dir`; after the loop, read `<scan_dir>/result.sarif` (non-empty) and list `<scan_dir>/findings/` to confirm your verdicts landed before you report.
4. **Pipeline boundary**: Only reason over the evidence packages returned by `secguard_plan`. Do NOT use `secguard_db` to query the `security_events` table or recover raw candidates that the convergence pipeline did not surface.
5. **Report**: Produce the report structure described under "Output Format" for the selected types. State explicitly which skills were executed and which were skipped. If any selected types failed during step 2, include a note indicating which type(s) failed and the error. **Before referencing the SARIF file**, verify it exists by reading `<scan_dir>/result.sarif` (written when your findings are persisted) — if missing or empty, do NOT reference it.
...
### Source paths
`report.md` shows paths relative to the scan target; the `## Location` block of each `candidates/<vuln-type>/NNN_*.md` carries the **absolute** path. Use that absolute path (or `files_with_candidates` from the scan summary) directly — do not reconstruct paths by trial and error.

### Skill namespace
Only load skills whose name is EXACTLY a `name` from `secguard types` (kebab-case, no prefix). Prefixed names like `crs-buffer-overflow` or `crs-crypto-misuse` belong to other products — never load them; if that is all you can find, stop and report it.
...
- `secguard_types` — **Type list tool**: Returns the current list of vulnerability types (`name` + `cwe`). Always call this first to discover/validate the type list; do not hardcode types or counts.
- `secguard_report` — Write findings (with `findings` arg; each finding may carry `summary`/`reasoning`/`exception_check`/`fix_strategy` — fill these for confirmed findings so the per-finding Markdown records *why* you believe it; returns each finding's `id` in the `written` array), record A5 second-round verdicts (with `reviews` arg: `id` + `review_status` + `review_reasoning`), or read all findings (no arg). Only findings with pipeline-supported CWE rule_ids are accepted. Findings for other CWE types are rejected — report those as observations in your summary text instead. **Always pass `scan_id` and `output_dir`**: they place each verdict file under `findings/<vuln-type>/` and regenerate `audit-report.md` + `result.sarif`, and the audit pass re-syncs `findings/` with the database (dismissed and unclassified entries are swept out). **Write one vulnerability type at a time** (incremental), not all types in one call. Then issue a `reviews` batch for every `suspected` finding (see Second-Round Confirmation).
