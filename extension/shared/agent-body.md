...
2. **问题总览表**（每类一行，用通俗名，不要 CWE 编号堆砌）：
   | 类别 | 确认 | 疑似 | 已排除误报 |

3. **问题清单表**（确认 + 疑似，每行一条）：
   | 类别 | 文件:行号 | 函数 | 严重度 | 结论 | 一句话说明 |

4. **修复建议**：对确认/疑似问题给出可落地的修复代码。

5. **逐条详情**：每条问题的证据与判定依据（引用源码片段）。

无发现时：只输出第 1 条结论 + 第 2 条总览表（"已排除误报"列填复核后排除的条数），然后一句"已排除的均为误报，判定依据见逐条详情"。
...
| 类别 | 文件:行号 | 函数 | 严重度 | 结论 | 说明 |
|------|-----------|------|--------|------|------|
| 缓冲区溢出 | src/parse.c:47 | parse_token | HIGH | 确认 | strlen 后 memcpy 无边界检查 |
| 空指针解引用 | src/alloc.c:42 | alloc_buf | HIGH | 确认 | malloc 返回值未判空即解引用 |
| 空指针解引用 | src/net.c:88 | recv_pkt | MEDIUM | 疑似 | 指针可能为 NULL，但存在部分校验 |
```
