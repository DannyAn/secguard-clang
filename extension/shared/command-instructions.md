...
3. **Process types one at a time** (per-type batch loop) to avoid context exhaustion:
   ...
   d. Write findings for THIS type only using `secguard_report` (incremental — one type at a time, not all types in one call). Pass `scan_id` and `output_dir` from the scan output. Include dismissed (false-positive) findings with a one-line evidence explaining why they are safe. For confirmed findings, fill `reasoning` + `exception_check` + `fix_strategy` so the per-finding Markdown carries the structured justification and fix (not just a one-liner).
...
2. **Per-type batch loop**: For each SELECTED vulnerability type only:
   ...
   e. Write findings for THIS type only using `secguard_report` (incremental). Pass `scan_id` and `output_dir` from step 1. For confirmed findings, fill `reasoning` + `exception_check` + `fix_strategy`.
