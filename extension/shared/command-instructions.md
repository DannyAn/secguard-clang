...
Instructions:
1. Run a full security scan on the target path using `secguard_scan`. The tool returns a summary (scan_id, output_dir, total_candidates, candidates_by_type) — NOT the full candidate list. Results are written to `.codeagent/secguard-clang/scans/<scan_id>/`: `report.md` + `candidates.sarif` (unclassified leads, level `note`) + `candidates/<vuln-type>/` (pipeline evidence). `findings/<vuln-type>/` and `result.sarif` are created from YOUR verdicts: `findings/` holds only confirmed/suspected entries, each filename carrying its verdict suffix, and dismissed findings get no file there. The database is stored at `.codeagent/secguard-clang/.sgre/sgre.db`.
...
3. **Process types one at a time** (per-type batch loop) to avoid context exhaustion:
   ...
   d. Write findings for THIS type only using `secguard_report` (incremental — one type at a time, not all types in one call). **Always pass `scan_id` and `output_dir`** — without them the verdict file cannot be placed under `findings/<vuln-type>/`. Persist dismissed (false-positive) findings too, with `reasoning` explaining why they are safe: excluding a batch in prose only leaves the exclusion unrecorded. For confirmed findings, fill `reasoning` + `exception_check` + `fix_strategy` so the verdict Markdown carries the structured justification and fix (not just a one-liner). If a write response returns `per_finding_warning`, fix the call and write again.
...
2. **Per-type batch loop**: For each SELECTED vulnerability type only:
   ...
   e. Write findings for THIS type only using `secguard_report` (incremental). Pass `scan_id` and `output_dir` from step 1 — both are required for the verdict file to be written. For confirmed findings, fill `reasoning` + `exception_check` + `fix_strategy`.
