...
生产环境扫描 805 文件 / 5438 函数 / 163533 行时发现的两类问题：null-deref 候选爆炸（1800+ 候选，最终只确认 80 条，端到端 90 分钟）与 unchecked-return 对 `(void)` cast 的 allocator-heuristic 函数误报。

#### unchecked-return（CWE-252）

- **`(void)` cast 对 allocator-heuristic 函数误报**：`HpeDynmemAlloc(id, &new_node)` 通过 out 参数分配内存且后续有判空，但名字含 "Alloc" 被 `apikb.IsAllocator` 启发式匹配，`(void)` cast 被忽略 → 误报。修复：新增 `isVoidCast(call)` helper，对 IsAllocator 启发式匹配的函数（非 `IsDeclaredAllocator`、非 `uncheckedReturnAPIs`、非 passthrough），`(void)` cast 跳过检测。`malloc`/`calloc`/`fopen` 等已知函数的 `(void)` cast 仍报。
...
- **顺带修复（第二处修法引入的新漏报）：`(A) & x` 位与被误判为取地址**。两种写法的 AST **完全相同**，语法上不可区分——**即使 typedef 就在同一个文件里也一样**（`(handle_t)&x` 与 `(flags) & mask` 同为 `binary_expression`）。仅凭"左操作数像类型"就接受，会把 `sink((flags) & mask)` 读成 `&mask`，静默吞掉 `mask` 的真实未初始化读取。新增 `parser.Node.AddressTakenTargetScoped(isValue)`：用**形参名 + 局部声明符名**构成作用域判别器区分类型名与值名（`parser.FunctionBoundNames`）；只收集 declarator、绝不收集类型名（实测 `typedef` 解析为 `type_definition`、声明里的 typedef 名解析为 `type_identifier`），因此不会把真取地址判成位与、让上面的 null-deref 误报复发。
- **同一判别器接入 planner（消除首轮遗留的不一致）**：`null_flow.addrTakenVar`（null-deref 流水线）与 `filter_uninit_flow.hasOutputParamWrite` 此前仍用宽松识别，于是 `p = NULL; f((flags) & p); *p` 这种写法在 null-deref 侧会被吞掉（漏报）。现在两者共用同一个作用域判别器（planner 从函数体 AST 现算，不依赖未落库的 `variables` 表），漏报消除；`flowAnalyzer` 按函数缓存判别器，热路径不重复遍历 AST。
...
- `report --write-json` 按 `(scan_id, rule_id, file, line, function)` UPSERT，同一
  file:line:function 的多个候选（如同一行两个资源变量 / 两个未初始化字段）会合并成一个
  finding；而 `scan_stats.final_count` 此前按「变量级」去重，导致 `status --per-type`
  与 `report --audit` 出现虚假的 `written < candidate` 差距（生产实测 24 条：6
  resource-leak + 18 uninit），orchestrator 误判为 context-overflow 并去拼
  `secguard db`/`secguard schema` 原始 SQL 排查。现 `final_count` 改为按
  `(file, line, function)` 位置去重（`distinctFindingLocations`），与 findings 的
  落库口径一致，`status`/`audit` 不再误报。
