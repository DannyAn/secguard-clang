...
`report.md` has **two stages**, the same split as SARIF: `secguard scan` writes
the candidate-stage report, and `report --audit` rewrites it as the verdict-stage
report once every candidate has a persisted verdict. Both stages open with the
same `## Scan Overview` block, so a reader can always tell how big the scan was:
...
current repository). Unknown figures (a scan predating the `scan_runs` metrics
table, a review run) render as `n/a` or are omitted — never as a misleading `0`.

Every one of these figures is derived from one place, `report.ScanOverview`
(`sgre/internal/report/overview.go`), which is also what renders
`audit-report.md`'s header and the `summary` field of the `report --audit` JSON
envelope. A console that echoes `summary` therefore cannot disagree with
`report.md`.

### audit-report.md

The per-skill pipeline statistics document, regenerated by `report --audit`. It
opens with the same `## Scan Overview` block and one-line headline as
`report.md`, then carries:

- `## Per-Skill Pipeline Statistics` — Seed / Final / Auto-confirmed /
  AI Confirmed / AI Suspected / AI Dismissed / Filter Efficiency / AI Accuracy
  per vulnerability type, plus a `TOTAL` row
- `## AI Value Summary` — raw seeds, auto-confirmed, candidates needing AI
  review, classified by AI, without AI classification, verdict counts, filter
  efficiency, AI accuracy
- `## Filter Chain Details` — the per-type filter chain and its drop counts

### `report --audit` JSON envelope (`summary`)

`secguard report --audit` returns `{scan_id, audit_path, sarif_path, report_path,
xlsx_path, vuln_count, findings_synced, status, audits, summary}` (`summary` is
also present on the no-`--output-dir` form). `audits` is the per-vulnerability-type
verdict breakdown; `summary` is the **scan-wide aggregate** rendered from the same
`report.ScanOverview` that wrote `report.md` and `audit-report.md`:

| Field | Meaning |
|-------|---------|
| `headline` | the one-sentence verdict, identical to `report.md`'s bold summary line |
| `scale` | human-readable scale label, e.g. `218 files / 5811 functions / 320450 lines` (or `… (in index)` when the scan wrote no metrics row) |
| `files_indexed` / `functions_indexed` | this scan's own walk figures |
| `files_in_index` / `functions_in_index` | the whole program graph, the fallback source for `scale` |
| `scan_metrics_available` | false when no `scan_runs` row exists for the scan (older scan, review run) |
...
(`auto_confirmed`) apart from AI verdicts, so a self-computed total can silently
...
| Col | Header | Source |
|-----|--------|--------|
| A | 序号 | 1..N, sorted by vuln type → file → line |
...
| I | 函数 | `function_name` |
| J | 问题摘要 | `summary` → `reasoning` → `evidence` → `function_name` |
| K | 详细分析 | `reasoning` (+ `[exception check] …` when present) |
| L | 修复建议 | `fix_strategy` |
| M | 代码上下文 | gutter-numbered source window, finding line marked `>` |

The sheet freezes the header row and adds an auto-filter over `A1:M{n+1}`.
The `代码上下文` column embeds the ±`--context-lines` window (default 15) in a
monospace font; `--context-lines 0` leaves it blank for repositories whose
source must not be copied into artifacts. `result.xlsx` shares `result.sarif`'s
lifecycle: it does not exist until `report --audit` runs, and it is regenerated
from the `findings` table (never accumulated in place).
