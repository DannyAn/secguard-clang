...
const (
	...
	// ResultXlsxFile is the Excel export regenerated by `report --audit` for the
	// development team: one row per CONFIRMED finding with a source-context
	// snippet, so confirmation can happen inside a spreadsheet without opening
	// the source tree. It is a one-way export (never read back).
	ResultXlsxFile = "result.xlsx"
	LatestName     = "latest"
	...
	// surface. Only CONFIRMED verdicts ever get a file here; a dismissed
	// (including undecidable) verdict is recorded in the DB only, never on the
	// review surface. See CandidatesDir for the pre-classification evidence.
